...
type AuthContextValue = {
  ...
  reloadProfileAndRoles: () => Promise<void>;
};
...
function friendlyAuthError(message?: string) {
  ...
  if (lower.includes('invalid login')) return 'Email or password does not match our records.';
  if (lower.includes('email not confirmed')) return 'Please confirm your email before logging in.';
  if (lower.includes('failed to fetch') || lower.includes('network')) return 'Network connection issue. Please try again.';

  return message;
}

function friendlyProfileError(message?: string) {
  if (!message) return 'SitGuru could not load your account profile yet.';

  const lower = message.toLowerCase();

  if (lower.includes('does not exist') || lower.includes('schema cache')) {
    return 'SitGuru could not find the expected profile or role fields yet.';
  }
...
function friendlyRoleError(message?: string) {
  if (!message) return 'SitGuru could not load your roles yet. Please refresh in a moment.';

  const lower = message.toLowerCase();

  if (lower.includes('permission') || lower.includes('policy') || lower.includes('rls')) {
    return 'Your account profile loaded, but SitGuru could not read your roles yet. Please refresh or contact support if this continues.';
  }

  if (lower.includes('failed to fetch') || lower.includes('network')) {
    return 'Your profile loaded, but roles could not refresh because of a network issue. Please try again.';
  }

  if (lower.includes('does not exist') || lower.includes('schema cache')) {
    return 'Your profile loaded, but SitGuru could not find the role records yet.';
  }

  return `Your profile loaded, but roles could not refresh: ${friendlyAuthError(message)}`;
}

function makeRoleOptions(roles: AppRole[]): RoleOption[] {
  return roles.map((role) => ({
    role,
    label: roleLabel(role),
    description: roleDescription(role),
    icon: roleIcon(role),
    dashboardPath: roleDashboardPath(role),
  }));
}
...
function fallbackName(user: User) {
  ...
  return full?.trim() || first?.trim() || user.email?.split('@')[0] || null;
}

function signupIntentRole(user: User) {
  return normalizeRole((user.user_metadata as Record<string, unknown>).signup_intent) ?? undefined;
}

function profileFromSafeRow(row: SafeProfileRow): ProfileSummary {
  return {
    id: row.id,
    email: row.email,
...
function profileFromFallbackRow(row: FallbackProfileRow, user: User): ProfileSummary {
  return {
    id: row.id,
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const loadProfileAndRoles = useCallback(
    async (user: User) => {
      ...
      try {
        ...
        if (error) {
          firstProfileError = error.message;
        } else if (data) {
          nextProfile = profileFromSafeRow(data as SafeProfileRow);
        }
      } catch (error) {
      ...
      if (!nextProfile && firstProfileError) {
        try {
          ...
          if (error) {
            nextError = friendlyProfileError(error.message);
          } else if (data) {
            nextProfile = profileFromFallbackRow(data as FallbackProfileRow, user);
          }
        } catch (error) {
      ...
      if (!nextProfile && !nextError) {
        nextError = 'Profile row was not found for this signed-in SitGuru account.';
      }
      ...
      try {
        ...
        if (error) {
          nextError = nextError ?? friendlyRoleError(error.message);
        } else if (Array.isArray(data)) {
          roleRows = data as UserRoleRecord[];
        }
      } catch (error) {
  ...
  const signUp = useCallback(
    async (email: string, password: string, metadata?: SignupMetadata) => {
      ...
      if (data.session?.user) {
        void loadProfileAndRoles(data.session.user);
      }
  ...
  const reloadProfileAndRoles = useCallback(async () => {
    if (session?.user) {
      await loadProfileAndRoles(session.user);
    } else {
      clearProfile();
    }
  }, [clearProfile, loadProfileAndRoles, session]);

  const primaryRole = roles[0] ?? null;
  const roleOptions = useMemo(() => makeRoleOptions(roles), [roles]);
