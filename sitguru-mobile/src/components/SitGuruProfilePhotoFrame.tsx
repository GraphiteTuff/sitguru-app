...
function dimensions(
  size: NonNullable<SitGuruProfilePhotoFrameProps['size']>,
  shape: NonNullable<SitGuruProfilePhotoFrameProps['shape']>,
) {
  const base = size === 'sm' ? 54 : size === 'md' ? 78 : size === 'lg' ? 116 : 244;

  if (shape === 'portrait') {
    return {
      width: size === 'hero' ? 260 : Math.round(base * 0.82),
      height: size === 'hero' ? 332 : Math.round(base * 1.18),
    };
  }
...
export default function SitGuruProfilePhotoFrame({
  imageUrl,
  name,
  email,
  roleLabel,
      ...
      <View style={[styles.frame, { borderRadius: radius, height: dim.height, width: dim.width }]}>
        {showImage ? (
          ...
          <View style={styles.fallback}>
            {fallbackEmoji ? (
              <Text style={[styles.emoji, { fontSize: Math.max(20, dim.width * 0.28) }]}>
                {fallbackEmoji}
              </Text>
            ) : null}
            ...
            <Text
              style={[
                styles.initials,
                {
                  fontSize: Math.max(14, dim.width * (shape === 'portrait' ? 0.18 : 0.24)),
                },
              ]}>
              {initials}
            </Text>
