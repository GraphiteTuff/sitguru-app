...
begin
  ...
  if duplicate_ambassadors is not null then
    raise exception
      'ambassadors unique index not created. Duplicate auth ids: %. Query: select user_id, count(*) from public.ambassadors group by 1 having count(*) > 1',
      duplicate_ambassadors;
  end if;
end $$;
...
create unique index if not exists ambassadors_one_per_auth_user
