...
export default function EventDetailShare({ event, partnerName }: EventDetailShareProps) {
  ...
  async function onShare() {
    const result = await shareEventNatively(shareEvent, "public_event_detail_drawer");
    if (result === "unavailable") setDrawerOpen(true);
  }
  ...
  return (
    <div id="share" className="mt-8">
      <button
        ...
        className="inline-flex min-h-12 w-full items-center justify-center gap-2 rounded-2xl bg-[#0D5C3A] px-6 text-sm font-black !text-white sm:w-auto"
      >
