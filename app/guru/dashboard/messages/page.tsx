...
async function fetchProfilesForGuruInbox({
  ...
  const profileColumns = "id, email, first_name, last_name, full_name, display_name, name, account_type, role, avatar_url, profile_photo_url, image_url";
...
function GuruDashboardMessagesPageContent() {
  ...
  const loadData = useCallback(
    async (preferredSelectedUser?: string | null) => {
      try {
        ...
        const safeMessages = ((messagesData || []) as Message[]).filter((message) => {
          const content = String(message.content || "").trim();
          const lowered = content.toLowerCase();

          return content && lowered !== "deleted" && lowered !== "[deleted]";
        });
  ...
  const filteredConversationItems = useMemo(() => {
    if (inboxFilter === "petParents") {
      return conversationItems.filter(
        (item) => isCustomerRole(item.profile) || item.isBookedCustomer
      );
    }

    if (inboxFilter === "admin") {
      return conversationItems.filter((item) => isAdminRole(item.profile));
    }

    if (inboxFilter === "unread") {
      return conversationItems.filter((item) => item.unreadCount > 0);
    }
      ...
      <div className="mx-auto max-w-7xl px-4 py-8 md:px-6 lg:px-8">
        <section className="overflow-hidden rounded-[2.25rem] border border-emerald-100 bg-white shadow-[0_18px_60px_rgba(15,23,42,0.08)]">
          ...
          <div className="grid gap-4 bg-white px-6 py-6 md:grid-cols-3 md:px-8">
            ...
            <div className="rounded-3xl bg-slate-50 p-5 ring-1 ring-slate-200">
              <p className="text-sm font-bold !text-slate-600">Pet Parents</p>
              <p className="mt-2 text-3xl font-extrabold !text-slate-950">
          ...
          <aside className="rounded-[2rem] border border-slate-200 bg-white p-4 shadow-sm">
            ...
            <div className="mb-4 grid grid-cols-2 gap-2">
              {[
                ...
                { key: "petParents", label: "Pet Parents", count: customerConversationCount },
                ...
                <button
                  ...
                  type="button"
                  onClick={() =>
                    setInboxFilter(tab.key as "all" | "petParents" | "admin" | "unread")
                  }
                  className={`rounded-2xl px-3 py-2 text-xs font-black transition ${
                    inboxFilter === tab.key
                      ? "bg-emerald-600 text-white"
                      : "bg-slate-50 !text-slate-700 ring-1 ring-slate-200 hover:bg-emerald-50"
                  }`}
                >
            ...
            <div className="space-y-2">
              ...
              {filteredConversationItems.length === 0 ? (
                <div className="rounded-[1.5rem] border border-dashed border-emerald-200 bg-emerald-50/60 p-5 text-sm font-semibold leading-6 !text-slate-800">
                  No Pet Parent messages yet. When a Pet Parent books you or starts a
        ...
        <section className="mt-6 grid gap-6 lg:grid-cols-3">
          <div className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm">
            ...
            <p className="mt-3 text-sm font-semibold leading-6 !text-slate-700">
              Booked Pet Parents and existing care conversations appear in your
