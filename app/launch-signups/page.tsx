...
function SummaryCard({
  ...
  return (
    <div className="rounded-3xl border border-white/10 bg-white/5 p-5 shadow-[0_10px_40px_rgba(0,0,0,0.18)]">
      ...
      <div className={`mt-3 inline-flex rounded-full border px-2.5 py-1 text-xs font-semibold ${toneClass}`}>
        Live
      </div>
  ...
  useEffect(() => {
    async function loadSignups() {
      ...
      try {
        ...
        const response = await fetch(`/api/launch-signup?${params.toString()}`, {
          cache: "no-store",
        });
