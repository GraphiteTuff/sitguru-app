...
export default async function AdminReferralPayoutsPage() {
  const [rewardsResult, partnerPayoutsResult, bookingsResult] = await Promise.all([
  ...
  const bookingCommissionSignals = bookingsResult.rows.filter((row) =>
    Boolean(firstString(row, ["partner_id", "partner_code", "affiliate_id", "affiliate_code", "ambassador_id", "ambassador_code", "referral_code"])),
  );
  const rows = [...rewards, ...partnerPayouts, ...bookingCommissionSignals];

  const pendingRows = rows.filter(isPending);
  const paidRows = rows.filter(isCompleted);

  return pageShell({
    eyebrow: "Admin / Referrals / Payouts",
    title: "Referral & Program Payouts",
    description:
      "Drill-down view for pending referral reward liability, issued care credits, Ambassador/Partner payouts, and program payment qualification.",
    icon: HandCoins,
    children: (
      <>
        <WarningBox warnings={[rewardsResult.warning, partnerPayoutsResult.warning, bookingsResult.warning].filter(Boolean) as string[]} />

        <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          <KpiCard label="Payout rows" value={formatNumber(rows.length)} detail="Referral reward, partner payout, and booking commission signal rows." icon={HandCoins} />
          <KpiCard label="Pending liability" value={money(pendingRows.reduce((sum, row) => sum + getAmount(row), 0))} detail="Pending, review, qualified, or approved reward/payout dollars." icon={BadgeDollarSign} />
          <KpiCard label="Issued / paid" value={money(paidRows.reduce((sum, row) => sum + getAmount(row), 0))} detail="Paid, credited, issued, or completed reward/payout dollars." icon={CheckCircle2} />
          <KpiCard label="Needs review" value={formatNumber(pendingRows.length)} detail="Rows that may need operational review before issuing reward or payout." icon={ClipboardList} />
        </section>

        <DataTable
          rows={rows}
          emptyText="No referral or program payout rows found yet."
          columns={[
            { label: "Recipient / Source", render: (row) => <div><p className="font-black text-slate-950">{getDisplayName(row, "Recipient")}</p><p className="text-xs text-slate-500">{getSource(row)}</p></div> },
            ...
            { label: "Open Financials", render: () => <Link href="/admin/financials/commissions" className="inline-flex items-center gap-2 font-black text-green-800">Commissions <ArrowRight className="h-4 w-4" /></Link> },
          ]}
        />
      </>
    ),
