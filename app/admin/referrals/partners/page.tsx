...
export default async function AdminPartnerReferralsPage() {
  const [partnersResult, applicationsResult, bookingsResult, payoutsResult, rewardsResult] = await Promise.all([
    safeRows<DbRow>(supabaseAdmin.from("partners").select("*").limit(5000), "partners"),
    safeRows<DbRow>(supabaseAdmin.from("partner_applications").select("*").limit(5000), "partner_applications"),
  ...
  const bookingRows = bookingsResult.rows.filter((row) => Boolean(firstString(row, ["partner_id", "partner_code", "affiliate_id", "affiliate_code"])));
  const payoutRows = payoutsResult.rows;
  const rewardRows = rewardsResult.rows.filter((row) => {
    const source = getSource(row).toLowerCase();
    return source.includes("partner") || source.includes("affiliate");
  });
  const rows = [...partnerRows, ...applications, ...bookingRows, ...rewardRows, ...payoutRows];

  return pageShell({
    eyebrow: "Admin / Referrals / Partners",
    title: "Partner Referrals",
    description:
      "Drill-down view for partner-sourced leads, applications, bookings, conversions, reward liability, and payout qualification.",
    icon: HeartHandshake,
    children: (
      <>
        <WarningBox warnings={[partnersResult.warning, applicationsResult.warning, bookingsResult.warning, payoutsResult.warning, rewardsResult.warning].filter(Boolean) as string[]} />

        <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          <KpiCard label="Partners" value={formatNumber(partnerRows.length)} detail="Partner records loaded from Supabase." icon={HeartHandshake} />
          <KpiCard label="Applications" value={formatNumber(applications.length)} detail="Partner application rows loaded." icon={ClipboardList} />
          <KpiCard label="Booking signals" value={formatNumber(bookingRows.length)} detail="Bookings with partner or affiliate source fields." icon={CheckCircle2} />
          <KpiCard label="Payout exposure" value={money([...payoutRows, ...rewardRows].reduce((sum, row) => sum + getAmount(row), 0))} detail="Partner payout and partner referral reward dollars found." icon={HandCoins} />
        </section>

        <DataTable
          rows={rows}
          emptyText="No Partner referral rows found yet."
          columns={[
            { label: "Partner / Email", render: (row) => <div><p className="font-black text-slate-950">{getDisplayName(row, "Partner")}</p><p className="text-xs text-slate-500">{getEmail(row) || "—"}</p></div> },
