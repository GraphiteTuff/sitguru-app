...
export default async function AdminReferralApplicationsPage() {
  const [programApplicationsResult, partnerApplicationsResult] = await Promise.all([
  ...
  const rows = [...programApplications, ...partnerApplications];

  const ambassadorRows = rows.filter((row) => getProgram(row).toLowerCase().includes("ambassador"));
  const studentRows = rows.filter((row) => getProgram(row).toLowerCase().includes("student"));
  const veteranRows = rows.filter((row) => {
    const program = getProgram(row).toLowerCase();
    return program.includes("veteran") || program.includes("military") || program.includes("skillbridge");
  });
  const partnerRows = rows.filter((row) => {
    const program = getProgram(row).toLowerCase();
    return program.includes("partner") || program.includes("affiliate");
  });
  ...
  return pageShell({
    ...
    children: (
      <>
        <WarningBox warnings={[programApplicationsResult.warning, partnerApplicationsResult.warning].filter(Boolean) as string[]} />
        ...
        <DataTable
          rows={rows}
          emptyText="No program application rows found yet."
          columns={[
            { label: "Applicant", render: (row) => <div><p className="font-black text-slate-950">{getDisplayName(row, "Applicant")}</p><p className="text-xs text-slate-500">{getEmail(row) || "—"}</p></div> },
            { label: "Program", render: (row) => getProgram(row) || "Program" },
            ...
            { label: "Open Programs", render: () => <Link href="/admin/programs" className="inline-flex items-center gap-2 font-black text-green-800">Programs <ArrowRight className="h-4 w-4" /></Link> },
          ]}
        />
      </>
    ),
