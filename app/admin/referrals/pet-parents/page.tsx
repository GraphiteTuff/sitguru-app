...
export default async function AdminPetParentReferralsPage() {
  const [profilesResult, bookingsResult, rewardsResult, eventsResult, codesResult] = await Promise.all([
    ...
    safeRows<DbRow>(supabaseAdmin.from("referral_events").select("*").limit(5000), "referral_events"),
    safeRows<DbRow>(supabaseAdmin.from("referral_codes").select("*").limit(5000), "referral_codes"),
  ]);

  const profiles = profilesResult.rows.filter((row) => hasReferralSignal(row) && (isCustomerProfile(row) || !isGuruProfile(row)));
  const bookings = bookingsResult.rows.filter(hasReferralSignal);
  const rewards = rewardsResult.rows;
  const events = eventsResult.rows;
  const codes = codesResult.rows;
  const rows = [...profiles, ...events, ...codes, ...rewards, ...bookings];

  return pageShell({
    eyebrow: "Admin / Referrals / Pet Parents",
    title: "Pet Parent Referrals",
    description:
      "Drill-down view for PawPerks referral-coded Pet Parent signups, invite codes, eligible booking activity, pending rewards, and issued care credits.",
    icon: Users,
    children: (
      <>
        <WarningBox warnings={[profilesResult.warning, bookingsResult.warning, rewardsResult.warning, eventsResult.warning, codesResult.warning].filter(Boolean) as string[]} />

        <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          <KpiCard label="Referral profiles" value={formatNumber(profiles.length)} detail="Pet Parent profile rows with referral signals." icon={Users} />
          <KpiCard label="Referral bookings" value={formatNumber(bookings.length)} detail="Bookings carrying referral, partner, ambassador, or invite signals." icon={CheckCircle2} />
          <KpiCard label="Pending rewards" value={money(rewards.filter(isPending).reduce((sum, row) => sum + getAmount(row), 0))} detail="Pending or review PawPerks reward liability." icon={BadgeDollarSign} />
          <KpiCard label="Issued rewards" value={money(rewards.filter(isCompleted).reduce((sum, row) => sum + getAmount(row), 0))} detail="Credited, issued, paid, or completed rewards." icon={HandCoins} />
        </section>

        <DataTable
          rows={rows}
          emptyText="No Pet Parent referral rows found yet."
          columns={[
            { label: "Name / Email", render: (row) => <div><p className="font-black text-slate-950">{getDisplayName(row, "Pet Parent")}</p><p className="text-xs text-slate-500">{getEmail(row) || "—"}</p></div> },
