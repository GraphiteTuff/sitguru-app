...
type ReferralRewardSummary = {
  ...
  issuedRows: ReferralRewardRow[];
  notQualifiedRows: ReferralRewardRow[];
  pendingAmount: number;
  issuedAmount: number;
  notQualifiedAmount: number;
  totalAmount: number;
  categoryRows: Array<{
    label: string;
    pending: number;
...
function asNumber(value: unknown) {
  ...
  if (typeof value === "string") {
    ...
    if (Number.isFinite(parsed)) return value.includes("(") ? -parsed : parsed;
  }
...
function buildByBookingMap(rows: DbRow[]) {
  ...
  for (const row of rows) {
    ...
    if (bookingId && !map.has(bookingId)) map.set(bookingId, row);
  }
  return map;
}
...
function getStatusBadgeClasses(status: string) {
  ...
  if (normalized.includes("captured")) return "border-sky-200 bg-sky-50 text-sky-800";
  if (normalized.includes("held") || normalized.includes("hold") || normalized.includes("review")) {
...
function getReferralRewardId(row: DbRow, index: number) {
  return (
    firstString(row, [
      "id",
      "reward_id",
...
function getReferralRewardAmount(row: DbRow) {
  return Math.abs(
    firstNumber(row, [
      "normalized_amount",
      "reward_amount",
...
function getReferralRewardStatus(row: DbRow) {
  return (
    firstString(row, ["normalized_status", "reward_status", "payout_status", "status"]) ||
    "pending"
  );
}

function getReferralFinancialTreatment(row: DbRow) {
  return firstString(row, ["financial_treatment", "statement_section"]) || "pending_reward_liability";
}

function getReferralFinancialCategory(row: DbRow) {
  return (
    firstString(row, [
      "financial_category",
      "reward_type",
...
function getReferralRelatedName(row: DbRow) {
  return (
    firstString(row, [
      "recipient_name",
      "referred_name",
...
function normalizeReferralRewardRow(row: DbRow, index: number): ReferralRewardRow {
  const category = getReferralFinancialCategory(row);
  const status = getReferralRewardStatus(row);
  const financialTreatment = getReferralFinancialTreatment(row);
  const createdAtRaw = getCreatedAt(row);
  const relatedName = getReferralRelatedName(row);

  return {
    id: getReferralRewardId(row, index),
    label: `${category} · ${relatedName}`,
    category,
    status,
    financialTreatment,
    amount: getReferralRewardAmount(row),
    createdAt: formatDateShort(createdAtRaw),
    createdAtRaw,
    source: firstString(row, ["source_table", "source", "referral_source", "type"]) || "referral_rewards",
    relatedName,
  };
}

function summarizeReferralRewards(rows: ReferralRewardRow[]): ReferralRewardSummary {
  const pendingRows = rows.filter((row) => row.financialTreatment === "pending_reward_liability");
  const issuedRows = rows.filter((row) => row.financialTreatment === "issued_reward_expense");
  const notQualifiedRows = rows.filter((row) => row.financialTreatment === "not_qualified");

  const categoryMap = new Map<
    string,
    { label: string; pending: number; issued: number; total: number; count: number }
  >();
  ...
  for (const row of rows) {
    ...
    if (row.financialTreatment === "pending_reward_liability") current.pending += row.amount;
    if (row.financialTreatment === "issued_reward_expense") current.issued += row.amount;
    current.total += row.amount;
    current.count += 1;
    categoryMap.set(row.category, current);
  }

  const sumRows = (items: ReferralRewardRow[]) => items.reduce((sum, row) => sum + row.amount, 0);
  ...
  return {
    ...
    issuedRows,
    notQualifiedRows,
    pendingAmount: sumRows(pendingRows),
    issuedAmount: sumRows(issuedRows),
    notQualifiedAmount: sumRows(notQualifiedRows),
    totalAmount: sumRows(rows),
    categoryRows: Array.from(categoryMap.values()).sort((a, b) => b.total - a.total),
  };
}
...
function buildQuery(params: SearchParamsShape, changes: Partial<SearchParamsShape>) {
  ...
  Object.entries(next).forEach(([key, value]) => {
    if (value && String(value).length > 0) query.set(key, String(value));
  });

  return `/admin/financials/commissions?${query.toString()}`;
}
...
function sortRows(rows: CommissionRow[], sort = "createdAt", dir: "asc" | "desc" = "desc") {
  return [...rows].sort((a, b) => {
    ...
    return dir === "asc" ? Number(aValue) - Number(bValue) : Number(bValue) - Number(aValue);
  });
}
...
function buildAnnualTrendPoints(rows: CommissionRow[]) {
  ...
  for (let month = 0; month < 12; month += 1) {
    const label = new Date(2026, month, 1).toLocaleDateString("en-US", { month: "short" });
...
async function getCommissionData(): Promise<CommissionData> {
  const [
    ...
    safeRows<DbRow>(supabaseAdmin.from("transactions").select("*").limit(1000), "transactions"),
    ...
    safeRows<DbRow>(
      supabaseAdmin.from("admin_referral_reward_liability").select("*").limit(1000),
  ...
  const capturedPayments = capturedRows.reduce((sum, row) => sum + row.customerPaymentAmount, 0);
  ...
  for (const row of commissionRows) {
    ...
    existing.bookings += 1;
    if (!existing.guruAvatarUrl && row.guruAvatarUrl) existing.guruAvatarUrl = row.guruAvatarUrl;
    topGuruMap.set(key, existing);
  }

  const topGurus = Array.from(topGuruMap.values()).sort((a, b) => b.guruNet - a.guruNet).slice(0, 5);
  const marketMap = new Map<string, MarketSummary>();

  for (const row of commissionRows) {
    const existing = marketMap.get(row.market) || { label: row.market, amount: 0, bookings: 0 };
    ...
    <div className="rounded-[1.4rem] border border-emerald-100 bg-white p-5 shadow-sm">
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-2 text-2xl font-black tracking-tight text-[#163127]">{value}</p>
  ...
  return (
    <div className="min-w-[170px] flex-1 rounded-[1.4rem] border border-emerald-100 bg-white p-4 shadow-sm">
      <div className="flex items-center gap-3">
        ...
        <div>
          <p className="text-[11px] font-black uppercase tracking-[0.12em] text-slate-400">{step}</p>
...
function SortHeader({
  ...
  return (
    ...
    >
      {label}
      {active ? <ArrowDown className="h-3.5 w-3.5" /> : <TrendingUp className="h-3.5 w-3.5 opacity-40" />}
    </Link>
  );
}
  ...
  return (
    <div className="rounded-[1.5rem] border border-emerald-100 bg-white p-5 shadow-sm">
      <div className="mb-5 flex items-center justify-between">
        ...
        <span className="rounded-full bg-emerald-50 px-3 py-1 text-xs font-black text-[#118a43]">Live</span>
      </div>

      <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
        {items.length ? (
          items.map((item, index) => (
            <div key={`${item.label}-${index}`} className="rounded-2xl border border-emerald-50 bg-[#fbfefc] p-4">
              <div className="flex items-start justify-between gap-4">
                <AvatarBadge
                  name={item.actorName}
                ...
                <div className="text-right">
                  <span className={`rounded-full border px-2.5 py-1 text-xs font-black ${getStatusBadgeClasses(item.status)}`}>
          ...
          <div className="rounded-2xl border border-dashed border-emerald-100 p-6 text-center md:col-span-2 xl:col-span-4">
            <p className="text-sm font-bold text-slate-500">No payment or payout activity found from Supabase yet.</p>
          </div>
        )}
      </div>
    </div>
...
function AnnualTrendChart({ points }: { points: TrendPoint[] }) {
  ...
  const maxValue = Math.max(...points.flatMap((point) => [point.gross, point.fees, point.net]), 1);
  const valueToY = (value: number) => topPadding + plotHeight - (value / maxValue) * plotHeight;
  const indexToX = (index: number) => leftPadding + index * (plotWidth / Math.max(points.length - 1, 1));
      ...
      <div className="mb-5 flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
        <div>
          <div className="flex flex-wrap items-center gap-2">
            <h2 className="text-xl font-black text-[#163127]">Sales, Fees & Net Payouts</h2>
        ...
        <div className="flex flex-wrap gap-3 rounded-2xl border border-emerald-100 bg-[#fbfefc] px-3 py-2 text-xs font-bold text-slate-500">
          <span className="flex items-center gap-1.5"><span className="h-2.5 w-2.5 rounded-full bg-[#118a43]" />Gross sales</span>
          <span className="flex items-center gap-1.5"><span className="h-2.5 w-2.5 rounded-full bg-blue-500" />Net payouts</span>
          <span className="flex items-center gap-1.5"><span className="h-2.5 w-2.5 rounded-full bg-orange-400" />Fees</span>
        </div>
      </div>
      ...
      <div className="h-[360px] overflow-hidden rounded-2xl border border-emerald-50 bg-gradient-to-b from-white to-[#f8fbf6] p-4">
        <svg viewBox={`0 0 ${chartWidth} ${chartHeight}`} className="h-full w-full">
          {yTicks.map((tick) => {
            ...
            return (
              <g key={tick}>
                <line x1={leftPadding} y1={y} x2={chartWidth - rightPadding} y2={y} stroke="#edf2e7" strokeWidth="1" />
                <text x={leftPadding - 12} y={y + 4} fill="#64748b" fontSize="11" fontWeight="800" textAnchor="end">
                  {moneyShort(tick)}
                </text>
              </g>
            );
          })}
          <line x1={leftPadding} y1={topPadding} x2={leftPadding} y2={topPadding + plotHeight} stroke="#dbe7d5" strokeWidth="1.5" />
          <line x1={leftPadding} y1={topPadding + plotHeight} x2={chartWidth - rightPadding} y2={topPadding + plotHeight} stroke="#dbe7d5" strokeWidth="1.5" />
          <polyline fill="none" stroke="#118a43" strokeWidth="5" strokeLinecap="round" strokeLinejoin="round" points={polyPoints("gross")} />
          <polyline fill="none" stroke="#3b82f6" strokeWidth="5" strokeLinecap="round" strokeLinejoin="round" points={polyPoints("net")} />
          <polyline fill="none" stroke="#fb923c" strokeWidth="5" strokeLinecap="round" strokeLinejoin="round" points={polyPoints("fees")} />
          {points.map((point, index) => (
            <g key={`${point.label}-${index}`}>
              <circle cx={indexToX(index)} cy={valueToY(point.gross)} r="4.5" fill="#118a43" stroke="white" strokeWidth="2.5" />
              <circle cx={indexToX(index)} cy={valueToY(point.net)} r="4" fill="#3b82f6" stroke="white" strokeWidth="2" />
              <circle cx={indexToX(index)} cy={valueToY(point.fees)} r="3.5" fill="#fb923c" stroke="white" strokeWidth="2" />
              <text x={indexToX(index)} y={topPadding + plotHeight + 26} fill="#64748b" fontSize="11" fontWeight="800" textAnchor="middle">
                {point.label}
              </text>
            </g>
          ))}
...
function RevenueFlowBreakdown({
  ...
  const rows = [
    { label: "Gross sales booked", description: "Total booking value before fees, refunds, holds, and payouts.", value: grossSales, tone: "bg-[#118a43]" },
    { label: "Customer payments captured", description: "Payments captured or completed from customers.", value: capturedPayments, tone: "bg-blue-500" },
    { label: "SitGuru platform fees", description: "Estimated or stored SitGuru service/application fee amount.", value: platformFees, tone: "bg-orange-400", negative: true },
    { label: "Partner commissions", description: "Partner, ambassador, affiliate, or referral commission amounts tied to bookings.", value: partnerCommissions, tone: "bg-violet-500", negative: true },
    { label: "Pending referral rewards", description: "PawPerks, Guru referral, Ambassador, and Partner rewards waiting to be issued.", value: referralRewardPending, tone: "bg-amber-400", negative: true },
    { label: "Issued referral rewards", description: "Referral rewards already credited, paid, or issued as an expense.", value: referralRewardIssued, tone: "bg-purple-500", negative: true },
    { label: "Refunds / disputes", description: "Refund and dispute dollars reducing the releasable payout pool.", value: refunds, tone: "bg-rose-500", negative: true },
    { label: "Guru net payout obligation", description: "Estimated or stored amount owed to Gurus after deductions.", value: guruNet, tone: "bg-emerald-400" },
    ...
    <div className="rounded-[1.75rem] border border-emerald-100 bg-white p-5 shadow-sm">
      <div className="mb-5 flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm font-semibold text-slate-500">
            Detailed view of how customer sales move through fees, commissions, referral rewards, payouts, holds, and release.
          </p>
        </div>
        <Link href="/admin/financials/payouts" className="inline-flex items-center gap-2 rounded-2xl border border-emerald-200 bg-[#f5fcf8] px-4 py-3 text-sm font-black text-[#118a43] transition hover:bg-[#ebf8f0]">
      ...
      <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4 2xl:grid-cols-6">
        {rows.map((row) => (
          <div key={row.label} className="rounded-2xl border border-emerald-50 bg-[#fbfefc] p-4">
            <p className="text-xs font-black uppercase tracking-[0.08em] text-slate-500">{row.label}</p>
            <p className="mt-2 text-xl font-black text-[#163127]">
              {row.negative ? "-" : ""}{moneyShort(row.value)}
            </p>
            <div className="mt-4 h-2.5 rounded-full bg-slate-100">
              <div className={`h-2.5 rounded-full ${row.tone}`} style={{ width: `${barWidth(row.value, max)}%` }} />
            </div>
            <p className="mt-3 text-xs font-semibold leading-5 text-slate-500">{row.description}</p>
          </div>
        ))}
      </div>
    </div>
...
function ReferralRewardsPanel({ summary }: { summary: ReferralRewardSummary }) {
  const maxCategoryValue = Math.max(...summary.categoryRows.map((row) => row.total), 1);

  return (
    <section className="rounded-[1.75rem] border border-emerald-100 bg-white p-5 shadow-sm">
      <div className="mb-5 flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
        <div>
          <h2 className="text-xl font-black text-[#163127]">Referral Reward & Program Payout Review</h2>
          <p className="mt-1 text-sm font-semibold text-slate-500">
            Pulls pending and issued PawPerks, Guru referral, Ambassador, and Partner reward liability from the financial view.
          </p>
        </div>
        <Link href="/admin/referrals" className="inline-flex items-center gap-2 rounded-2xl border border-emerald-200 bg-[#f5fcf8] px-4 py-3 text-sm font-black text-[#118a43] transition hover:bg-[#ebf8f0]">
          Open Growth & Referrals <ArrowRight className="h-4 w-4" />
        </Link>
      </div>

      <div className="grid gap-4 md:grid-cols-3">
        <KpiCard label="Pending Reward Liability" value={moneyShort(summary.pendingAmount)} detail={`${summary.pendingRows.length.toLocaleString()} rewards waiting to be issued`} icon={<AlertTriangle className="h-5 w-5" />} tone="bg-amber-50 text-amber-600" />
        <KpiCard label="Issued Reward Expense" value={moneyShort(summary.issuedAmount)} detail={`${summary.issuedRows.length.toLocaleString()} issued, credited, or paid rewards`} icon={<CheckCircle2 className="h-5 w-5" />} tone="bg-emerald-50 text-[#118a43]" />
        <KpiCard label="Referral Reward Rows" value={summary.rows.length.toLocaleString()} detail={`${summary.notQualifiedRows.length.toLocaleString()} not qualified / rejected rows`} icon={<Users className="h-5 w-5" />} tone="bg-violet-50 text-violet-600" />
      </div>

      <div className="mt-6 grid gap-5 xl:grid-cols-[1fr_1fr]">
        <div className="rounded-2xl border border-emerald-50 bg-[#fbfefc] p-4">
          <p className="text-sm font-black text-[#163127]">Reward category mix</p>
              ...
              summary.categoryRows.map((row) => (
                <div key={row.label}>
                  <div className="mb-2 flex items-center justify-between gap-3">
                    <div>
                      <p className="text-sm font-black text-[#163127]">{row.label}</p>
                      <p className="text-xs font-semibold text-slate-500">
                        {row.count} row{row.count === 1 ? "" : "s"} · {moneyShort(row.pending)} pending · {moneyShort(row.issued)} issued
                      </p>
                    </div>
                    <p className="text-sm font-black text-[#163127]">{moneyShort(row.total)}</p>
                  </div>
                  <div className="h-2.5 rounded-full bg-slate-100">
                    <div className="h-2.5 rounded-full bg-violet-500" style={{ width: `${barWidth(row.total, maxCategoryValue)}%` }} />
                  </div>
                </div>
              ))
            ) : (
              <div className="rounded-2xl border border-dashed border-emerald-100 p-6 text-center">
                <p className="text-sm font-bold text-slate-500">No referral reward rows found yet.</p>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="rounded-2xl border border-emerald-50 bg-[#fbfefc] p-4">
          <p className="text-sm font-black text-[#163127]">Recent referral reward rows</p>
          <div className="mt-4 divide-y divide-emerald-50 overflow-hidden rounded-2xl border border-emerald-50 bg-white">
            {summary.rows.slice(0, 6).map((row) => (
              <div key={row.id} className="grid gap-3 p-4 sm:grid-cols-[1fr_auto] sm:items-center">
                <div>
                  <p className="text-sm font-black text-[#163127]">{row.relatedName}</p>
                  <p className="mt-1 text-xs font-semibold text-slate-500">{row.category} · {row.status} · {row.createdAt}</p>
                </div>
                <div className="text-left sm:text-right">
                  <p className="text-sm font-black text-[#163127]">{money(row.amount)}</p>
                  <span className={`mt-1 inline-flex rounded-full border px-2.5 py-1 text-[11px] font-black uppercase tracking-[0.1em] ${
                    row.financialTreatment === "issued_reward_expense"
                      ? "border-emerald-200 bg-emerald-50 text-emerald-800"
                      : row.financialTreatment === "not_qualified"
                        ? "border-slate-200 bg-slate-100 text-slate-700"
                        : "border-amber-200 bg-amber-50 text-amber-800"
                  }`}>{row.financialTreatment.replaceAll("_", " ")}</span>
                </div>
              </div>
            ))}
            {summary.rows.length === 0 ? (
              <div className="p-6 text-center text-sm font-bold text-slate-500">
                Referral rewards will appear here after PawPerks, Ambassador, Partner, or Guru referral rows are created.
              </div>
            ) : null}
          </div>
        </div>
...
function DonutCard({
  ...
  const total = Math.max(platformFees + partnerCommissions + referralRewards + guruNet + heldAmount, 1);
  ...
  const referralPct = calcPercent(referralRewards, total);
  ...
  const rows = [
    ...
    ["Referral Rewards", referralRewards, referralPct, "bg-amber-400"],
    ...
    <div className="rounded-[1.5rem] border border-emerald-100 bg-white p-5 shadow-sm">
      <div className="mb-5 flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-xs font-semibold text-slate-500">Real distribution of payout and reward dollars.</p>
        </div>
        <Link href="/admin/financials/payouts" className="text-xs font-black text-[#118a43] hover:text-[#0c7338]">View breakdown</Link>
      </div>
        ...
        <div
          ...
          style={{
            background: `conic-gradient(
              ...
              #f59e0b ${guruPct + platformPct + partnerPct}% ${guruPct + platformPct + partnerPct + referralPct}%,
              #f43f5e ${guruPct + platformPct + partnerPct + referralPct}% 100%
            )`,
          }}
        >
            ...
            <div key={label} className="flex items-center gap-3">
              ...
              <div className="flex-1">
                <div className="flex items-center justify-between gap-3">
                  ...
                  <p className="text-sm font-black text-[#163127]">{moneyShort(Number(value))}</p>
                </div>
                <p className="text-xs font-semibold text-slate-500">{Number(pct).toFixed(1)}% of payout mix</p>
              </div>
            </div>
          ))}
        </div>
  ...
  return (
    <div className="rounded-[1.5rem] border border-emerald-100 bg-white p-5 shadow-sm">
      <div className="mb-5 flex items-start justify-between gap-4">
        ...
        <Link href={href} className="text-xs font-black text-[#118a43] hover:text-[#0c7338]">View all</Link>
      </div>
      {children}
    </div>
  );
...
function LedgerTable({ rows, params }: { rows: CommissionRow[]; params: SearchParamsShape }) {
  return (
        ...
        <table className="min-w-[1320px] w-full">
          <thead className="bg-[#fbfefc]">
            <tr className="border-b border-emerald-100 text-left">
              <th className="px-5 py-4 text-xs font-black uppercase tracking-[0.08em]"><SortHeader label="Booking" sortKey="booking" params={params} /></th>
              <th className="px-5 py-4 text-xs font-black uppercase tracking-[0.08em]"><SortHeader label="Customer" sortKey="customer" params={params} /></th>
              <th className="px-5 py-4 text-xs font-black uppercase tracking-[0.08em]"><SortHeader label="Guru" sortKey="guru" params={params} /></th>
              <th className="px-5 py-4 text-xs font-black uppercase tracking-[0.08em]"><SortHeader label="Market" sortKey="market" params={params} /></th>
              <th className="px-5 py-4 text-xs font-black uppercase tracking-[0.08em]"><SortHeader label="Gross" sortKey="gross" params={params} /></th>
              <th className="px-5 py-4 text-xs font-black uppercase tracking-[0.08em]"><SortHeader label="Platform Fee" sortKey="fee" params={params} /></th>
              <th className="px-5 py-4 text-xs font-black uppercase tracking-[0.08em]"><SortHeader label="Partner Commission" sortKey="partner" params={params} /></th>
              <th className="px-5 py-4 text-xs font-black uppercase tracking-[0.08em]"><SortHeader label="Guru Net" sortKey="net" params={params} /></th>
              <th className="px-5 py-4 text-xs font-black uppercase tracking-[0.08em]"><SortHeader label="Status" sortKey="status" params={params} /></th>
              <th className="px-5 py-4 text-xs font-black uppercase tracking-[0.08em]"><SortHeader label="Date" sortKey="createdAt" params={params} /></th>
            </tr>
          </thead>
          <tbody>
            {rows.slice(0, 50).map((row) => (
              <tr key={row.id} className="border-b border-emerald-50 transition hover:bg-[#fafefb]">
                ...
                <td className="px-5 py-4 align-top"><AvatarBadge name={row.customerName} subtitle="Customer payment" avatarUrl={row.customerAvatarUrl} tone="blue" /></td>
                <td className="px-5 py-4 align-top"><AvatarBadge name={row.guruName} subtitle="Guru payout" avatarUrl={row.guruAvatarUrl} tone="green" /></td>
                <td className="px-5 py-4 align-top"><p className="font-bold text-[#163127]">{row.market}</p></td>
                <td className="px-5 py-4 align-top font-black text-[#163127]">{money(row.grossAmount)}</td>
                <td className="px-5 py-4 align-top font-black text-[#118a43]">{money(row.platformFee)}</td>
                <td className="px-5 py-4 align-top font-black text-violet-700">{money(row.partnerCommission)}</td>
                <td className="px-5 py-4 align-top font-black text-[#163127]">{money(row.guruNet)}</td>
                <td className="px-5 py-4 align-top"><span className={`inline-flex rounded-full border px-2.5 py-1 text-xs font-black ${getStatusBadgeClasses(row.ledgerStatus)}`}>{getStatusLabel(row.ledgerStatus)}</span></td>
                <td className="px-5 py-4 align-top font-bold text-slate-600">{row.createdAt}</td>
              </tr>
            ))}
            {rows.length === 0 && (
              <tr>
                <td colSpan={10} className="px-5 py-14 text-center">
                  <p className="text-lg font-black text-[#163127]">No real commission rows found</p>
                  <p className="mt-2 text-sm text-slate-500">Add bookings, payments, and payout records in Supabase to populate this page.</p>
                </td>
              </tr>
            )}
          </tbody>
  ...
  return (
    <div className="mx-auto w-full max-w-[1480px] space-y-6 px-4 pb-10 pt-1 text-slate-900 sm:px-6 xl:px-8">
        ...
        <div className="flex flex-col gap-5 xl:flex-row xl:items-start xl:justify-between">
          <div>
            <div className="flex flex-wrap items-center gap-2 text-sm font-semibold text-slate-500">
              <Link href="/admin" className="transition hover:text-[#118a43]">Admin</Link>
              <span>›</span>
              <Link href="/admin/financials" className="transition hover:text-[#118a43]">Financials</Link>
              <span>›</span>
      ...
      <section className="rounded-[2rem] border border-emerald-100 bg-white p-5 shadow-sm">
        <div className="mb-4 flex items-center justify-between">
          <div>
            ...
            <p className="mt-1 text-sm font-medium text-slate-500">Complete real flow from booking to customer payment, fees, commissions, rewards, and net release.</p>
          </div>
          <Link href="/admin/financials/payouts" className="text-sm font-black text-[#118a43] hover:text-[#0c7338]">View flow analytics →</Link>
        </div>
        <div className="flex gap-3 overflow-x-auto pb-1">
          <FlowCard step="01" label="Gross Sales" value={moneyShort(commissions.totals.grossSales)} detail={`${commissions.totals.bookings.toLocaleString()} bookings`} icon={<CalendarDays className="h-5 w-5" />} tone="bg-emerald-50 text-[#118a43]" />
          <FlowCard step="02" label="Customer Payments" value={moneyShort(commissions.totals.capturedPayments)} detail={`${percent(capturedRate)} captured`} icon={<CreditCard className="h-5 w-5" />} tone="bg-sky-50 text-sky-600" />
          <FlowCard step="03" label="Platform Fees" value={moneyShort(commissions.totals.platformFees)} detail={`${percent(commissions.totals.takeRate)} take rate`} icon={<HandCoins className="h-5 w-5" />} tone="bg-orange-50 text-orange-600" />
      ...
      <RevenueFlowBreakdown
        ...
        referralRewardPending={commissions.totals.referralRewardPendingAmount}
        referralRewardIssued={commissions.totals.referralRewardIssuedAmount}
      ...
      <section className="grid gap-6 xl:grid-cols-3">
        <DonutCard
          platformFees={commissions.totals.platformFees}
          partnerCommissions={commissions.totals.partnerCommissions}
          referralRewards={commissions.totals.referralRewardTotalAmount}
          guruNet={commissions.totals.guruNetPayouts}
          heldAmount={commissions.totals.heldAmount}
        />

        <TopListCard title="Top 5 Markets by Payout" description="Real market-level payout distribution from booking/customer location fields." href="/admin/analytics">
          <div className="space-y-4">
            {commissions.topMarkets.map((market, index) => (
              <div key={market.label}>
                <div className="mb-2 flex items-center justify-between gap-3">
                  <div className="flex items-center gap-3">
                    <span className="flex h-8 w-8 items-center justify-center rounded-2xl bg-emerald-50 text-sm font-black text-[#118a43]">{index + 1}</span>
                    <div><p className="text-sm font-black text-[#163127]">{market.label}</p><p className="text-xs font-semibold text-slate-500">{market.bookings} bookings</p></div>
                  </div>
                  <p className="text-sm font-black text-[#163127]">{moneyShort(market.amount)}</p>
                </div>
                <div className="h-2.5 rounded-full bg-slate-100"><div className="h-2.5 rounded-full bg-[#118a43]" style={{ width: `${barWidth(market.amount, commissions.maxMarket)}%` }} /></div>
              </div>
            ))}
            {commissions.topMarkets.length === 0 ? <div className="rounded-2xl border border-dashed border-emerald-100 p-6 text-center"><p className="text-sm font-bold text-slate-500">No market payout data found yet.</p></div> : null}
          </div>
        </TopListCard>

        <TopListCard title="Top 5 Gurus by Net Earnings" description="Highest real Guru net payout value." href="/admin/gurus">
          <div className="space-y-4">
            {commissions.topGurus.map((guru, index) => (
              <div key={guru.guruId}>
                <div className="mb-2 flex items-center justify-between gap-3">
                  <div className="flex min-w-0 items-center gap-3">
                    <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-2xl bg-emerald-50 text-sm font-black text-[#118a43]">{index + 1}</span>
                    <AvatarBadge name={guru.guruName} subtitle={`${guru.bookings} bookings`} avatarUrl={guru.guruAvatarUrl} tone="green" size="sm" />
                  </div>
                  <p className="text-sm font-black text-[#163127]">{moneyShort(guru.guruNet)}</p>
                </div>
                <div className="h-2.5 rounded-full bg-slate-100"><div className="h-2.5 rounded-full bg-[#118a43]" style={{ width: `${barWidth(guru.guruNet, commissions.maxGuruNet)}%` }} /></div>
              </div>
            ))}
            {commissions.topGurus.length === 0 ? <div className="rounded-2xl border border-dashed border-emerald-100 p-6 text-center"><p className="text-sm font-bold text-slate-500">No Guru earnings found yet.</p></div> : null}
          </div>
        </TopListCard>
      </section>
      ...
      <section className="rounded-[2rem] border border-emerald-100 bg-white p-5 shadow-sm">
        <form action="/admin/financials/commissions" className="space-y-4">
          <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
            <div className="flex items-start gap-3">
              <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl bg-emerald-50 text-[#118a43]"><Filter className="h-5 w-5" /></div>
              <div><h2 className="text-2xl font-black text-[#163127]">Organize Commission Data</h2><p className="mt-1 text-sm text-slate-500">Search, filter, and sort real booking-level commission and payout rows.</p></div>
            </div>
            <div className="flex flex-wrap gap-3">
              <button type="submit" className="inline-flex items-center justify-center gap-2 rounded-2xl bg-[#118a43] px-4 py-3 text-sm font-black text-white transition hover:bg-[#0c7338]">Apply Filters <Filter className="h-4 w-4" /></button>
              <Link href="/admin/financials/commissions" className="inline-flex items-center justify-center gap-2 rounded-2xl border border-emerald-200 bg-[#f5fcf8] px-4 py-3 text-sm font-black text-[#118a43] transition hover:bg-[#ebf8f0]">Reset <RefreshCw className="h-4 w-4" /></Link>
            </div>
          </div>
          ...
          <div className="grid gap-3 lg:grid-cols-[1.5fr_1fr_1fr]">
            <label className="relative block">
              ...
              <input type="text" name="q" defaultValue={params.q || ""} placeholder="Search booking, customer, guru, market, status..." className="h-12 w-full rounded-2xl border border-emerald-100 bg-[#fcfffd] pl-11 pr-4 text-sm font-semibold text-[#163127] outline-none transition placeholder:text-slate-400 focus:border-emerald-300 focus:ring-4 focus:ring-emerald-50" />
            </label>
            <select name="status" defaultValue={params.status || "all"} className="h-12 w-full rounded-2xl border border-emerald-100 bg-[#fcfffd] px-4 text-sm font-semibold text-[#163127] outline-none transition focus:border-emerald-300 focus:ring-4 focus:ring-emerald-50">
              <option value="all">All Statuses</option><option value="paid">Paid</option><option value="pending">Pending</option><option value="unpaid">Unpaid</option><option value="held">Held</option><option value="failed">Failed</option>
            </select>
            <select name="source" defaultValue={params.source || "all"} className="h-12 w-full rounded-2xl border border-emerald-100 bg-[#fcfffd] px-4 text-sm font-semibold text-[#163127] outline-none transition focus:border-emerald-300 focus:ring-4 focus:ring-emerald-50">
              <option value="all">All Types</option><option value="guru">Guru Net</option><option value="platform">Platform Fees</option><option value="partner">Partner Commissions</option><option value="referral">Referral Rewards</option><option value="refund">Refunds / Disputes</option>
            </select>
          </div>
        </form>
      </section>
      ...
      <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        ...
        <Link href="/admin/financials/payment-gateway" className="rounded-[1.4rem] border border-emerald-100 bg-white p-5 shadow-sm transition hover:-translate-y-0.5 hover:shadow-md"><CreditCard className="h-5 w-5 text-[#118a43]" /><p className="mt-3 text-base font-black text-[#163127]">Payment Gateway</p><p className="mt-1 text-sm text-slate-500">Track Stripe, PayPal, Apple Pay, Google Pay, Venmo, and Plaid banking by provider.</p></Link>
      ...
      <section className="rounded-[2rem] border border-emerald-100 bg-white p-5 shadow-sm">
        ...
        <div className="mt-5 grid gap-3 md:grid-cols-2 xl:grid-cols-4 2xl:grid-cols-6">
          {[
            ...
            ["Referral Rewards", commissions.sourceCounts.referralRewards],
          ].map(([label, value]) => (
