        ...
        <section className="rounded-[2rem] border border-emerald-100 bg-white p-5 shadow-[0_22px_60px_rgba(15,23,42,0.08)] sm:p-7 lg:p-8">
          ...
          <div className="mt-6 grid gap-3 sm:grid-cols-2">
            ...
            <button
              ...
              disabled={googleLoading || appleLoading}
              className="inline-flex min-h-12 items-center justify-center gap-3 rounded-2xl border border-slate-900 bg-slate-950 px-4 py-3 text-sm font-black text-white transition hover:bg-black disabled:cursor-not-allowed disabled:opacity-70"
            >
              {appleLoading ? (
                <Loader2 className="h-5 w-5 animate-spin" />
              ) : (
          ...
          <div className="mt-5 flex items-center gap-3 text-[11px] font-black uppercase tracking-[0.16em] text-slate-400">
            <span className="h-px flex-1 bg-slate-200" />
            or use {loginMethod === "email" ? "email" : "phone"}
            <span className="h-px flex-1 bg-slate-200" />
          </div>
