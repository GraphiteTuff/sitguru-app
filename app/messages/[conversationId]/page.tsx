...
export default async function MessageConversationPage({
  ...
  return (
    <main className="flex min-h-dvh flex-col bg-[#f7fcfa] text-slate-950">
      <Header />
      <MessageAutoRefresh intervalMs={1000} />
      <MessageRealtimeRefresh conversationId={conversation.id} currentUserId={user.id} />

      <section className="mx-auto flex w-full max-w-3xl flex-1 flex-col px-3 pb-[max(0.5rem,env(safe-area-inset-bottom))] pt-3 sm:px-6">
        <header className="flex shrink-0 items-center gap-3">
          <Link
            href={inboxHref}
            className="inline-flex h-12 w-12 items-center justify-center rounded-2xl border border-emerald-100 bg-white text-emerald-800"
        ...
        <div className="mt-3 min-h-0 flex-1 overflow-y-auto overscroll-contain rounded-[1.5rem] border border-emerald-100 bg-white p-3 sm:p-4">
          {safeMessages.length === 0 ? (
            ...
            <div className="space-y-3">
              {safeMessages.map((message) => {
                ...
                <div
                  id="reply"
                  className="sticky bottom-0 mt-3 shrink-0 rounded-[1.5rem] border border-emerald-100 bg-white p-3 shadow-sm"
                >
