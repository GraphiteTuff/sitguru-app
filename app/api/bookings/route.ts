...
function getFirstBoolean(
  ...
  for (const key of keys) {
    if (key in body) return toBoolean(body[key], fallback);
  }
...
function normalizePaymentOptionLabel(value: string) {
  const normalized = value.trim().toLowerCase();

  if (!normalized) return "Not selected";
  if (["card", "credit_card", "debit_card", "credit/debit", "credit_debit_card"].includes(normalized)) {
    return "Credit / debit card";
  }
  if (["apple_pay", "apple pay", "applepay"].includes(normalized)) return "Apple Pay";
  if (["google_pay", "google pay", "googlepay"].includes(normalized)) return "Google Pay";
  if (["link", "link_by_stripe", "stripe_link", "link by stripe"].includes(normalized)) return "Link by Stripe";
  if (["saved", "saved_card", "saved_payment_method", "saved method"].includes(normalized)) {
    return "Saved payment method";
  }
  if (["ach", "bank", "bank_account", "us_bank_account", "bank transfer"].includes(normalized)) {
    return "ACH / bank account";
  }
  if (["quote", "custom_quote", "request_quote", "quote_request"].includes(normalized)) {
...
function getPaymentOptionSummary({
  ...
  const parts = [paymentMethodLabel || selectedPaymentOption || "Not selected"];

  if (pawperksCreditRequested) parts.push("PawPerks requested");
  if (referralCreditRequested) parts.push("Referral credit requested");
  if (promoCode) parts.push(`Promo: ${promoCode}`);
  if (giftCardCode) parts.push("Gift/SitGuru credit entered");
  if (sitguruCreditRequested) parts.push("SitGuru credit requested");
  if (tipAmount > 0) parts.push(`Guru tip: $${tipAmount.toFixed(2)}`);
...
export async function POST(req: NextRequest) {
  try {
    ...
    const selectedPaymentOption = getFirstText(body, [
      "selectedPaymentOption",
    ...
    const paymentMethodLabel =
      getFirstText(body, ["paymentMethodLabel", "payment_method_label"]) ||
      normalizePaymentOptionLabel(selectedPaymentOption);
    const paymentProvider = getFirstText(body, [
      "paymentProvider",
      "payment_provider",
      "checkoutProvider",
      "checkout_provider",
    ], "Stripe");
    const paymentWalletType = getFirstText(body, [
      "paymentWalletType",
      "payment_wallet_type",
      "walletType",
      "wallet_type",
    ]);
    const savedPaymentMethodRequested = getFirstBoolean(body, [
      "savedPaymentMethodRequested",
      "saved_payment_method_requested",
      "useSavedPaymentMethod",
      "use_saved_payment_method",
    ], selectedPaymentOption.toLowerCase().includes("saved"));
    const achBankRequested = getFirstBoolean(body, [
      "achBankRequested",
      "ach_bank_requested",
      "bankAccountRequested",
      "bank_account_requested",
    ], ["ach", "bank", "us_bank_account"].some((value) => selectedPaymentOption.toLowerCase().includes(value)));
    const pawperksCreditRequested = getFirstBoolean(body, [
      "pawperksCreditRequested",
      "pawperks_credit_requested",
      "petperksCreditRequested",
      "petperks_credit_requested",
      "usePawPerksCredit",
      "use_pawperks_credit",
    ]);
    const referralCreditRequested = getFirstBoolean(body, [
      "referralCreditRequested",
      "referral_credit_requested",
      "useReferralCredit",
      "use_referral_credit",
    ]);
    const sitguruCreditRequested = getFirstBoolean(body, [
      "sitguruCreditRequested",
      "sitguru_credit_requested",
      "useSitGuruCredit",
      "use_sitguru_credit",
      "giftCardCreditRequested",
      "gift_card_credit_requested",
    ]);
    const promoCode = getFirstText(body, [
      "promoCode",
      "promo_code",
      "discountCode",
      "discount_code",
    ]);
    const giftCardCode = getFirstText(body, [
      "giftCardCode",
      "gift_card_code",
      "sitguruCreditCode",
      "sitguru_credit_code",
    ]);
    const pawperksCreditAmount = getFirstNumber(body, [
      "pawperksCreditAmount",
      "pawperks_credit_amount",
      "petperksCreditAmount",
      "petperks_credit_amount",
    ], 0);
    const referralCreditAmount = getFirstNumber(body, [
      "referralCreditAmount",
      "referral_credit_amount",
    ], 0);
    const sitguruCreditAmount = getFirstNumber(body, [
      "sitguruCreditAmount",
      "sitguru_credit_amount",
      "giftCardAmount",
      "gift_card_amount",
    ], 0);
    const quoteRequestStatus = getFirstText(body, [
      "quoteRequestStatus",
      "quote_request_status",
    ], selectedPaymentOption.toLowerCase().includes("quote") ? "requested" : "not_requested");
    ...
    const resolvedGuruId =
      getFirstText(guru || {}, ["id", "profile_id", "user_id"]) || incomingGuruId;
    ...
    const paymentOptionSummary = getPaymentOptionSummary({
      selectedPaymentOption,
      paymentMethodLabel,
      pawperksCreditRequested,
      referralCreditRequested,
    ...
    const bookingPayload: Record<string, unknown> = {
      ...
      notes: [
        ...
        `Payment option selected: ${paymentOptionSummary}`,
        promoCode ? `Promo code entered: ${promoCode}` : "",
        giftCardCode ? "Gift card / SitGuru credit code entered." : "",
        pawperksCreditRequested ? "PawPerks credit requested." : "",
        referralCreditRequested ? "Referral credit requested." : "",
        sitguruCreditRequested ? "SitGuru credit requested." : "",
        tipAmount > 0
      ...
      selected_payment_option: selectedPaymentOption || null,
      payment_option: selectedPaymentOption || null,
      payment_method_label: paymentMethodLabel,
      payment_provider: paymentProvider,
      payment_wallet_type: paymentWalletType || null,
      saved_payment_method_requested: savedPaymentMethodRequested,
      ach_bank_requested: achBankRequested,
      pawperks_credit_requested: pawperksCreditRequested,
      petperks_credit_requested: pawperksCreditRequested,
      referral_credit_requested: referralCreditRequested,
      sitguru_credit_requested: sitguruCreditRequested,
      promo_code: promoCode || null,
      promo_code_entered: promoCode || null,
      gift_card_code: giftCardCode || null,
      sitguru_credit_code: giftCardCode || null,
      pawperks_credit_amount: pawperksCreditAmount,
      referral_credit_amount: referralCreditAmount,
      sitguru_credit_amount: sitguruCreditAmount,
      payment_option_summary: paymentOptionSummary,
      checkout_option_summary: paymentOptionSummary,
      quote_request_status: quoteRequestStatus,
      custom_quote_requested: quoteRequestStatus === "requested",
      ...
      status: "pending",
      payment_status: quoteRequestStatus === "requested" ? "quote_requested" : "unpaid",
