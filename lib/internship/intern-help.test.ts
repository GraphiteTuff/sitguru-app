...
describe("intern help catalog", () => {
  ...
  it("finds student wording like check-in, hours, and tracking links", () => {
    ...
    const paypal = searchInternHelpArticles("paypal");
    assert.ok(paypal.some((article) => article.slug === "payments-on-sitguru"));
    const applePay = searchInternHelpArticles("apple pay");
    assert.ok(applePay.some((article) => article.slug === "payments-on-sitguru"));
    const venmo = searchInternHelpArticles("venmo");
    assert.ok(venmo.some((article) => article.slug === "payments-on-sitguru"));
    const plaid = searchInternHelpArticles("plaid");
    assert.ok(plaid.some((article) => article.slug === "payments-on-sitguru"));
    const premium = searchInternHelpArticles("premium payments");
    assert.ok(premium.some((article) => article.slug === "payments-on-sitguru"));
    const paymentsArticle = internHelpArticle("payments-on-sitguru");
    assert.equal(paymentsArticle?.shots[0]?.file, "assets/premium-payments.png");
    assert.match(paymentsArticle?.purpose || "", /Rover or Wag/);
    assert.doesNotMatch(JSON.stringify(paymentsArticle), /jasongraff1978@gmail\.com/i);
    assert.doesNotMatch(paymentsArticle?.purpose || "", /\bJason\b/);
  ...
  it("serves training screenshots from intern Help media, never Your page", () => {
    ...
    assert.equal(internHelpMediaAllowed("assets/premium-payments.png"), true);
