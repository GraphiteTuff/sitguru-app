  ...
  const [liveWalks, walkSessions, gpsEvents] = await Promise.all([
    ...
    safeSelectCascade([
      ...
      },
      {
        ...
        columns: "id,recorded_at,created_at",
        limit: 50,
        sinceIso: since,
...
async function moduleCashFlow(since: string): Promise<ModuleSnapshot> {
  ...
  const outflow = sumField(payoutRows, ["amount", "payout_amount", "amount_cents"]);
...
async function modulePayouts(since: string): Promise<ModuleSnapshot> {
  ...
  const [payouts, guruPayouts] = await Promise.all([
    ...
    safeSelect(
      "guru_payouts",
      "id,amount,amount_cents,status,created_at",
      400,
      since,
    ),
  ]);
  const rows = normalizePayoutRows(
    payouts.ok ? payouts.rows : guruPayouts.rows,
  );
  ...
  return finalize(
    ...
    `${number(pending.length)} payouts pending (${money(sumField(pending, ["amount", "amount_cents"]))}) · ${number(paid.length)} paid.`,
    {
      pendingCount: pending.length,
      pendingAmount: sumField(pending, ["amount", "amount_cents"]),
      paidCount: paid.length,
      paidAmount: sumField(paid, ["amount", "amount_cents"]),
      source: payouts.ok ? "payouts" : "guru_payouts",
    },
    [
      `${number(pending.length)} pending`,
      `${money(sumField(pending, ["amount", "amount_cents"]))} queued`,
    ],
